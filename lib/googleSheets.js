...
export async function writeToPersonalGrid(sheets, spreadsheetId, sheetTitle, { categoria, monto, mesNombre, anio, headerRowIdx, catColIdx }) {
  ...
  let rowIdx = findRowIndex(values, catColIdx, categoria, { startRow: headerRowIdx + 1, exact: false });
  let categoriaUsada = categoria;
  if (rowIdx === -1) {
    // La categoría puede existir en el sheet compartido (Perris, Servicios, Carniceria...)
    // pero no tener fila propia en esta grilla personal. El usuario pidió (26/06/2026) que
    // en ese caso, en vez de fallar, se cargue en la fila "Varios" de la grilla personal.
    rowIdx = findRowIndex(values, catColIdx, "varios", { startRow: headerRowIdx + 1, exact: false });
    if (rowIdx !== -1) categoriaUsada = values[rowIdx]?.[catColIdx];
  }
  if (rowIdx === -1) {
  ...
  return { fila: rowIdx + 1, columna: colToLetter(monthCol), categoriaUsada, valorAnterior: current, valorNuevo: nuevo };
}
