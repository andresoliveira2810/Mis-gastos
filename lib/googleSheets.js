...
// con el más a la derecha (asumiendo que es el más reciente / vigente).
...
export async function writeToPersonalGrid(sheets, spreadsheetId, sheetTitle, { categoria, monto, mesNombre, headerRowIdx, catColIdx }) {
  ...
  const monthCol = findColIndex(headerRow, mesNombre, { onlyVisible: true, isColHidden, exact: true });
  if (monthCol === -1) {
    const err = new Error(
