...
function mesNombreToIdx(mesNombre) {
  const i = MESES_IDX.indexOf(normalize(mesNombre));
  return i === -1 ? null : i + 1; // 1-12
}

// Convierte un serial de fecha de Sheets (días desde 30/12/1899, lo que devuelve la API
// con valueRenderOption=UNFORMATTED_VALUE para una celda con formato de fecha) a {mes, anio}.
...
// Algunos sheets (ej. la grilla de Clarita) usan como encabezado de columna una FECHA real
// (ej. 01/06/2026, que Sheets muestra como "06/2026") en vez del texto "Junio". Bug
// confirmado el 26/06/2026: con esos headers, findColIndex/findAllColIndices (que comparan
// texto) nunca encuentran nada, y la app reporta (incorrectamente) que la columna del mes
// no existe o está oculta. Esta función busca columnas cuyo valor es una fecha que cae en
// el mes/año buscado, como fallback cuando no hubo match de texto.
function findDateColIndices(headerRow, mesIdx, anio, { onlyVisible = false, isColHidden = () => false } = {}) {
  const out = [];
  for (let c = 0; c < headerRow.length; c++) {
    if (onlyVisible && isColHidden(c)) continue;
    const my = serialToMesAnio(headerRow[c]);
    if (my && my.mes === mesIdx && my.anio === anio) out.push(c);
  }
  return out;
}

export function findMonthColIndex(headerRow, mesNombre, anio, { onlyVisible = false, isColHidden = () => false } = {}) {
  let matches = findAllColIndices(headerRow, mesNombre, { onlyVisible, isColHidden, exact: true });

  if (matches.length === 0) {
    const mesIdx = mesNombreToIdx(mesNombre);
    if (mesIdx) {
      matches = findDateColIndices(headerRow, mesIdx, Number(anio) || new Date().getFullYear(), { onlyVisible, isColHidden });
    }
  }
...
export async function writeToPersonalGrid(
  sheets,
  spreadsheetId,
  sheetTitle,
  { categoria, monto, mesNombre, anio, headerRowIdx, catColIdx, detalle, variablesCategoria }
) {
  ...
  const esCategoriaVariable = variablesCategoria && normalize(categoria) === normalize(variablesCategoria);

  // Caso normal: la categoría tiene su propia fila en la grilla y NO es la categoría
  // "variable" configurada (esa celda es una fórmula, nunca se pisa directo — ver abajo).
  if (!esCategoriaVariable) {
    const rowIdx = findRowIndex(values, catColIdx, categoria, { startRow: headerRowIdx + 1, exact: false });
    if (rowIdx !== -1) {
      const categoriaEncontrada = values[rowIdx]?.[catColIdx];
      const esLaFilaVariable = variablesCategoria && normalize(categoriaEncontrada) === normalize(variablesCategoria);
      if (!esLaFilaVariable) {
        ...
        return {
          destino: "grilla",
          fila: rowIdx + 1,
          columna: colToLetter(monthCol),
          categoriaUsada: categoriaEncontrada,
  ...
  // Categoría "variable" explícita, o sin fila propia en la grilla (incluyendo el caso en
  // que el único match por texto fue justo la fila variable): si hay un bloque "Variables"
  // configurado, itemizamos ahí (pedido del usuario 26/06/2026: "Mis gastos varios van a
  // la derecha. Si hay alguno que no sabes dónde va ponelo ahí").
  if (variablesCategoria) {
    const block = findVariablesBlock(values, headerRowIdx, mesNombre, anio, { onlyVisible: true, isColHidden });
    if (block) {
      const r = await appendVariableExpense(sheets, spreadsheetId, sheetTitle, {
        labelCol: block.labelCol,
        montoCol: block.montoCol,
        detalle: detalle || categoria,
        monto,
        headerRowIdx,
      });
      return { destino: "variables", ...r, categoriaUsada: variablesCategoria };
    }
    // No encontramos el bloque "Variables" para este mes (ej. el usuario todavía no
    // armó esa columna para el mes que viene) — preferimos fallar explícito antes que
    // arriesgarnos a escribir en la celda-fórmula "Gastos varios" por error.
    const err = new Error(
      `La hoja "${sheetTitle}" tiene categoría variable configurada pero no encontré el bloque "Variables" para ${mesNombre} ${anio}. Revisá que esa columna exista también en el bloque de la derecha.`
    );
    err.code = "VARIABLES_BLOCK_NOT_FOUND";
    throw err;
  }

  // Sin bloque "Variables" configurado (ej. la grilla de Clarita): comportamiento viejo,
  // cargar en la fila "Varios" de la grilla personal si existe.
  const rowIdxVarios = findRowIndex(values, catColIdx, "varios", { startRow: headerRowIdx + 1, exact: false });
  if (rowIdxVarios !== -1) {
    const categoriaUsada = values[rowIdxVarios]?.[catColIdx];
    const current = toNumber(values[rowIdxVarios]?.[monthCol]);
    const nuevo = current + monto;
    await updateCell(sheets, spreadsheetId, sheetTitle, rowIdxVarios, monthCol, nuevo);
    return { destino: "grilla", fila: rowIdxVarios + 1, columna: colToLetter(monthCol), categoriaUsada, valorAnterior: current, valorNuevo: nuevo };
  }

  const err = new Error(`No encontré la categoría "${categoria}" ni una categoría "Varios" en la hoja "${sheetTitle}".`);
  err.code = "CATEGORY_NOT_FOUND";
  throw err;
}

// ─── dominio: bloque "Variables" (itemizado a la derecha de la grilla personal) ───────
// Algunas grillas (ej. la planilla nueva de Andrés, 26/06/2026) tienen, a la derecha de
// la grilla fija categoría×mes, un bloque que itemiza los gastos variables del mes en vez
// de un solo monto. Hay un par de columnas (label + monto) POR MES, repetido tantas veces
// como meses tenga la grilla. La columna "label" de cada instancia mensual tiene, en
// headerRowIdx, el mismo serial de fecha que la grilla principal, y en headerRowIdx+1 el
// texto "Variables" — eso es lo que usamos para encontrarla sin asumir números de columna
// fijos. El TOTAL de cada instancia alimenta vía fórmula la celda "Gastos varios" de la
// grilla principal, por eso esa celda nunca se escribe directo.
export function findVariablesBlock(values, headerRowIdx, mesNombre, anio, { onlyVisible = false, isColHidden = () => false } = {}) {
  const subHeaderRow = values[headerRowIdx + 1] || [];
  const headerRow = values[headerRowIdx] || [];
  const mesIdx = mesNombreToIdx(mesNombre);
  if (!mesIdx) return null;
  const anioReal = Number(anio) || new Date().getFullYear();

  for (let c = 0; c < subHeaderRow.length; c++) {
    if (onlyVisible && isColHidden(c)) continue;
    if (normalize(subHeaderRow[c]) !== "variables") continue;
    const my = serialToMesAnio(headerRow[c]);
    if (my && my.mes === mesIdx && my.anio === anioReal) {
      return { labelCol: c, montoCol: c + 1 };
    }
  }
  return null;
}

// Itemiza un gasto nuevo en el bloque "Variables": busca la primera fila libre entre el
// sub-header y la fila "TOTAL" (cualquier fila con texto en la columna label — incluidos
// los rótulos reservados "F1"-"F4" — se considera ocupada, así nunca pisamos nada existente
// sin tener que saber qué significa cada rótulo). Si el bloque está lleno, insertamos una
// fila nueva justo arriba del TOTAL, acotada a estas 2 columnas (insertRange en vez de
// insertDimension) para que la fórmula SUMA() se expanda sola y el resto de la hoja —
// incluida la grilla principal — quede totalmente intacto.
export async function appendVariableExpense(sheets, spreadsheetId, sheetTitle, { labelCol, montoCol, detalle, monto, headerRowIdx }) {
  const snap = await getTabSnapshot(sheets, spreadsheetId, sheetTitle);
  const { values, sheetId: gid } = snap;

  const totalRow = findRowIndex(values, labelCol, "TOTAL", { startRow: headerRowIdx + 2, exact: true });
  if (totalRow === -1) {
    const err = new Error(`No encontré la fila "TOTAL" del bloque Variables en la columna ${colToLetter(labelCol)}.`);
    err.code = "VARIABLES_TOTAL_NOT_FOUND";
    throw err;
  }

  let targetRow = -1;
  for (let r = headerRowIdx + 2; r < totalRow; r++) {
    const cell = values[r]?.[labelCol];
    if (cell === undefined || cell === null || String(cell).trim() === "") {
      targetRow = r;
      break;
    }
  ...
  await writeRow(sheets, spreadsheetId, sheetTitle, targetRow, labelCol, [detalle || "", Math.abs(monto)]);

  return { fila: targetRow + 1, columnaLabel: colToLetter(labelCol), columnaMonto: colToLetter(montoCol) };
}

// ─── dominio: sheet compartido (una sola tabla, fila MONTO + filas "real") ──
// El usuario unificó las dos tablas viejas ("Principio del mes" / "Resto del mes")
// en una sola (confirmado inspeccionando el sheet real el 25/06/2026): ahora hay una
// única fila de categorías (fila 5, la que sigue diciendo "Principio del mes" en la
// columna A — quedó como título de la tabla única), una fila "MONTO" (fila 6) con el
// consumo de cada categoría, filas "Andrés %"/"Clara %" (filas 7/9) que el sheet
// calcula solo con fórmulas — nunca las tocamos — y las filas "Andrés real" (fila 8)
// / "Clara real" (fila 10) donde va el mismo monto de la fila MONTO, según quién pagó.
// A fin de mes el usuario borra los valores a mano y vuelve a cargar desde cero — por
// eso buscamos las filas por texto en vez de por número de fila fijo, así seguimos
// funcionando aunque el usuario inserte o borre alguna fila más adelante.
//
// Bug confirmado el 25/06/2026: la versión vieja de esta función buscaba una tabla
// "Resto del mes" que ya no existe (el usuario la unificó con "Principio del mes"),
// y por eso cualquier gasto compartido en una categoría que antes pertenecía a esa
// segunda tabla (Servicios, Supermercado, Verduleria, Carniceria, Perris, Varios)
// fallaba con "No encontré la tabla 'Resto del mes' en el sheet compartido." — el
// gasto personal sí se guardaba bien, por eso la app mostraba "Guardado parcialmente".
...
export async function writeSharedExpense(sheets, sharedSheetId, sheetTitle, { categoria, monto, usuarioKey }) {
  ...
  const headerRow = findRowIndex(values, 0, "Principio del mes", { exact: false });
  if (headerRow === -1) {
    throw new Error('No encontré la fila de encabezado ("Principio del mes") en el sheet compartido.');
  }

  const catCol = findColIndex(values[headerRow] || [], categoria, { exact: true });
  if (catCol === -1) {
    throw new Error(`La categoría "${categoria}" no está en las columnas del sheet compartido.`);
  }

  const montoRow = findRowIndex(values, 0, "MONTO", { startRow: headerRow + 1, exact: true });
  if (montoRow === -1) {
    throw new Error('No encontré la fila "MONTO" en el sheet compartido.');
  }

  const nombreSheet = NOMBRE_EN_SHARED_SHEET[usuarioKey] || usuarioKey;
  const realRow = findRowIndex(values, 0, `${nombreSheet} real`, { startRow: headerRow + 1, exact: true });
  if (realRow === -1) {
    throw new Error(`No encontré la fila "${nombreSheet} real" en el sheet compartido.`);
  }
  ...
  return { categoria, filaMonto: montoRow + 1, filaReal: realRow + 1, montoNuevo: nuevoMonto, realNuevo: nuevoReal };
}
